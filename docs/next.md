...
상태: **M5 완료 + 실사용 안정화·성능 튜닝 완료 · 오디오 먹싱은 보류** ·
...
- [ ] 오디오+비디오 동시 먹싱이 왜 실 플레이어에서 깨지는지 근본 원인 미해결 — 필요해지면
      재검토(다른 오디오 인코더/컨테이너, 또는 오디오를 별도 RTSP 경로로 분리하는 방향 등)
- [ ] MULTI 모드 풀바디 줌 재검증 — 카메라-피사체 거리 확보되는 환경에서
...
- [ ] 오디오 먹싱 — 배선은 됐지만 실 플레이어에서 재생이 깨져 보류. 위 "오디오 먹싱" 섹션 참고
...
1. 오디오 먹싱 근본 원인 — 필요해지면 재검토(위 섹션 참고), 지금은 비디오 전용으로 운영
