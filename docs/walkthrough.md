...
`paths: all_others:` 없이 빈 설정으로 띄우면 임의 경로 publish가 400/404로 막힌다.
`rtspTransports: [tcp]`는 처음엔 없었는데(UDP가 로컬망이라 더 저지연이라는 이유로 기본값
그대로 뒀음), 실사용 중(오디오 먹싱 테스트) UDP RTP 패킷 손실/재정렬로 보이는 문제를
여럿 겪었다 — OBS Media Source가 검은 화면만 보여주거나(RTP depacketizer가 mediamtx의
재조각화된 패킷을 못 견딤), 재생 시간은 흐르는데 화면이 첫 프레임에 멈추는 증상(디코더가
이후 패킷을 못 받음). `output.py`도 `-rtsp_transport tcp`로 맞춰 publish한다 — 로컬호스트
지연 차이는 무시할 만한데 안정성 이득이 훨씬 크다.
