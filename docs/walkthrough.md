...
mediamtx mediamtx.yml   # 저장소 루트의 설정 파일 (git에 커밋돼 있음)
reframe --src 3 --mode 1 --no-preview --rtsp-out rtsp://localhost:8554/out1
```

[mediamtx.yml](../mediamtx.yml)의 `rtspTransports: [tcp]`는 처음엔 없었는데(UDP가 로컬망이라
더 저지연이라는 이유로 기본값 그대로 뒀음), 실사용 중(오디오 먹싱 테스트) UDP RTP 패킷
손실/재정렬로 보이는 문제를 여럿 겪었다 — OBS Media Source가 검은 화면만 보여주거나(RTP
depacketizer가 mediamtx의 재조각화된 패킷을 못 견딤), 재생 시간은 흐르는데 화면이 첫
프레임에 멈추는 증상(디코더가 이후 패킷을 못 받음). 다른 컴퓨터에 새로 설치할 때 이 설정이
빠지면 처음부터 똑같은 버그를 다시 겪게 되므로 `/tmp`가 아니라 저장소에 커밋해뒀다.
`output.py`도 `-rtsp_transport tcp`로 맞춰 publish한다 — 로컬호스트 지연 차이는 무시할
만한데 안정성 이득이 훨씬 크다.
...
```bash
mediamtx mediamtx.yml   # RTSP 쓸 거면 (1b 참고)
...
**오디오는 reframe 스트림에 먹싱하지 않고 OBS에서 별도 트랙으로 얹는 게 정석이다.** reframe는
같은 소스를 4개 독립 비디오 채널로 내보내고 OBS가 그중 하나를 전환하며 송출하므로, 오디오는
채널과 무관한 하나의 연속 트랙이어야 한다(먹싱하면 4벌 복제 + 채널 전환 시 끊김). OBS에서
마이크를 `오디오 입력 캡처`로 추가하고, 비디오가 파이프라인 지연으로 늦게 오니 `고급 오디오
속성 → 동기화 오프셋`에 양수(ms)를 넣어 맞춘다. **NDI 소스로 받으면 지연이 ~400ms로 작고
일정**해 한 번 맞춘 오프셋이 계속 유효(RTSP는 더 크고 흔들림). 오프셋은 모니터링이 아니라
**녹화 파일 재생**으로 확인할 것(모니터링엔 오프셋이 적용 안 됨). 상세: [next.md](next.md)
"실운영 해법: 오디오는 OBS에서 별도 트랙으로" 섹션.

(참고) `--audio-src <avfoundation 오디오 장치 인덱스>`로 reframe 스트림 자체에 오디오를 먹싱하는
배선도 남아있지만(장치 목록은 `python3 -c "import sources; print(sources.probe_audio_devices())"`),
...
- reframe 스트림 내 오디오 먹싱 — 배선(§1d `--audio-src`)은 됐지만 실 플레이어 재생 문제로
  보류. **오디오는 OBS 별도 트랙으로 운영**(§1d 참고)하므로 reframe 자체는 비디오 전용
- NDI 채널의 오디오 — reframe의 NDI 출력은 비디오 전용(cyndilib에 실시간 오디오 캡처를
  연결하려면 별도 라이브러리 필요). 어차피 오디오는 OBS에서 마이크를 직접 잡으므로 불필요
...
마우스 조작까지 확인 완료. Cam Link 4K로 실사용하며 해상도 기본값·머리 잘림·`zoom=manual`
되먹임·UI 클릭 반응 없음 등 여러 버그를 추가로 잡았고, 4채널 프레임 쓰기를 병렬화해
**30fps를 실측으로 확인**했다(상세는 next.md "M5 실사용 안정화 + 30fps 튜닝" 섹션 참고).
