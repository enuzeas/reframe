...
| M6 | 서비스화 | ✅ 완료(2026-07-04) — 단일 프로세스 설계라 서비스는 **2종**(mediamtx+reframe-server, 계획서의 4-프로세스 분리는 유예 유지). `scripts/install-services.sh`가 머신별 절대경로를 해석해 launchd plist 2개 생성(plist는 커밋 안 함) + Homebrew bin을 서버 PATH에 주입(launchd 기본 PATH엔 ffmpeg 없음). KeepAlive+기존 퍼블리셔 재시도로 기동 순서 처리. `persistence.py`로 채널 배치를 `state.json`에 원자적·디바운스 저장/복원(`target_id`·입력소스는 세션 스코프라 제외). **실동작 검증**: 편집→하드 재시작→복원, RTSP out1 h264 1080p 서비스 PATH에서 인코딩 확인 | 재부팅 후 자동 기동(완료), 마지막 채널 배치 복원(완료) | M5 | INFRA-PLAN.md §5, §8, §11 |
| M7 (옵션) | 확장 | BoT-SORT+ReID · NDI(분리형 시나리오 B) · 4채널 동시 녹화 | 필요 발생 시에만 착수 | M6 | PLAN.md §3.2 / INFRA-PLAN.md §1(B) / PLAN.md Phase3 |

## 지금 다음 액션

M0~M6 핵심 검증 끝났다 — 4K 카메라로 줌/패닝 검증, 4채널 컨트롤 서버, 크롭 편집/트래킹
바인딩 API 연동, 실사용 버그 수정, 30fps 성능 튜닝, **launchd 서비스화+채널 배치 영속화**까지
완료. 오디오는 reframe 먹싱 대신 **OBS 별도 트랙**으로 운영하기로 확립(NDI 수신 ~400ms,
동기화 오프셋 한 번 맞춤). 남은 건 MULTI 풀바디 줌 재검증(거리 확보되는 환경),
M7(옵션: BoT-SORT+ReID / 4채널 녹화 등, 필요 발생 시) — 상세는 [next.md](next.md) 참조.
