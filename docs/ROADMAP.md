...
| M3 | 송출 PoC (1채널) | 🟡 부분 완료(2026-07-04) — RTSP 경로 실동작 확인(`output.py` 신규, UDP+B프레임0 튜닝), 실사용 중 버그 2건 발견·수정(줌 클램프, One Euro min_cutoff). NDI 경로도 추가(`NDIPublisher`, Syphon 대신 — 이 머신엔 obs-ndi/NDI SDK가 이미 있어 마찰 적음), mDNS 디스커버리로 네트워크 레벨 확인 완료, OBS 화면 육안 확인만 남음. **줌/패닝 시각 검증은 노트북 웹캠 화각 한계로 보류 — 실 4K 카메라 필요.** 오디오먹싱 미착수 | RTSP·NDI 경로 동작 확인(완료) · 줌/패닝 육안 확인(4K 카메라 대기) | M2 | UI-PLAN.md §4, 상세: [next.md](next.md) |
