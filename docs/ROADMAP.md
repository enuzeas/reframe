...
| M3 | 송출 PoC (1채널) | 🟡 부분 완료(2026-07-04) — RTSP·NDI 두 경로 모두 실동작 확인(`output.py`에 `RTSPPublisher`+`NDIPublisher`, UDP+B프레임0 튜닝). NDI는 Syphon 대신 채택(이 머신엔 obs-ndi/NDI SDK가 이미 있어 마찰 적음) — mDNS 디스커버리부터 실제 웹캠 프레임이 OBS NDI Source 화면에 뜨는 것까지 육안 확인 완료. 실사용 중 버그 2건 발견·수정(줌 클램프, One Euro min_cutoff). **줌/패닝 시각 검증은 노트북 웹캠 화각 한계로 보류 — 실 4K 카메라 필요.** 오디오먹싱 미착수 | RTSP·NDI 경로 동작 확인(완료) · 줌/패닝 육안 확인(4K 카메라 대기) | M2 | UI-PLAN.md §4, 상세: [next.md](next.md) |
| M4 | 4채널 확장 + 컨트롤 서버 | encode 프로세스 4채널화 · FastAPI+WebSocket · 프리뷰 스트림+캔버스 오버레이(읽기 전용 콘솔) | 4채널 동시 송출 + 읽기 전용 콘솔에서 실시간 인물박스/프리뷰 확인 | M3 | UI-PLAN.md §6 (기존 step 2-3) |
...
M0·M1·M2 끝났다. **M3는 4K 카메라가 생기기 전까지 여기서 일시정지** — 남은 항목(Syphon PoC,
오디오 먹싱, 줌/패닝 육안 검증)은 하드웨어 의존적이라 지금 더 진행해도 검증이 안 된다.
4K 카메라 없이도 할 수 있는 다음 작업은 [next.md](next.md) 참조.
