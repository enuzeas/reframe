...
| M3 | 송출 PoC (1채널) | 🟡 부분 완료(2026-07-04) — RTSP 경로 실동작 확인(`output.py` 신규, UDP+B프레임0 튜닝), 실사용 중 버그 2건 발견·수정(줌 클램프, One Euro min_cutoff). **줌/패닝 시각 검증은 노트북 웹캠 화각 한계로 보류 — 실 4K 카메라 필요.** Syphon/오디오먹싱 미착수 | RTSP 경로 동작 확인(완료) · 줌/패닝 육안 확인(4K 카메라 대기) | M2 | UI-PLAN.md §4, 상세: [next.md](next.md) |
