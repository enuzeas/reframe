...
바인딩 API 연동, 오디오 먹싱 배선까지 완료. 남은 건 오디오 실 청취 확인, MULTI 풀바디 줌
재검증(거리 확보되는 환경), M6(서비스화) — 상세는 [next.md](next.md) 참조.
